class Piece {
  constructor(i, j, troopType, team) {
    ...
    if (team == 'white') {
      this.image = images[troopNames.indexOf(troopType) + 1];
    } else {
      this.image = images[troopNames.indexOf(troopType) + 1 + troopNames.length];
    }
  ...
  findPossibleMoves(canHaveMistakes) {
    let moves = [];
    let i = this.i;
    let j = this.j;
    ...
    switch (this.type) {
      case 'pawn':
        if (this.team == 'white' && i > 0) {
          if (getPiece(i - 1, j) == null) moves.push([i - 1, j]);
          if (i == 6 && getPiece(4, j) == null && getPiece(5, j) == null) moves.push([i - 2, j]);
          let piece1 = getPiece(i - 1, j - 1);
          let piece2 = getPiece(i - 1, j + 1);
          if (piece1 != null && piece1.team == 'black') moves.push([i - 1, j - 1]);
          if (piece2 != null && piece2.team == 'black') moves.push([i - 1, j + 1]);

        } else if (i < 7) {
          if (getPiece(i + 1, j) == null) moves.push([i + 1, j]);
          if (i == 1 && getPiece(3, j) == null && getPiece(2, j) == null) moves.push([i + 2, j]);
          let piece1 = getPiece(i + 1, j - 1);
          let piece2 = getPiece(i + 1, j + 1);
    ...
    if (canHaveMistakes == true) {
      for (let i = moves.length - 1; i >= 0; i--) {
        let move = moves[i];
        if (willBeCheckedIf(this, move[0], move[1])) moves.splice(i, 1);
      }
    }
  ...
  move(di, dj) {
    ...
    if (canMove) {
      let attackedPiece = getPiece(di, dj);
      if (attackedPiece != null && attackedPiece.type == 'king') return false;

      printMove(this, this.i, this.j, di, dj);

      this.place(di, dj);

      if (attackedPiece != null) {
        removeTroop(attackedPiece);
      }
    }
